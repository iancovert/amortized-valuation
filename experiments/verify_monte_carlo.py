...
train_filenames = glob(os.path.join('data_files', dataset, f'mc-{num_points}-{num_points}-*.npy'))
gt_filenames = [filename for filename in all_filenames if filename not in train_filenames]
...
gt_first_results = [np.load(filename, allow_pickle=True).item() for filename in gt_first]
gt_second_results = [np.load(filename, allow_pickle=True).item() for filename in gt_second]
train_results = [np.load(filename, allow_pickle=True).item() for filename in train_filenames]
...
for count in sample_counts:
    ...
    aggregated_results = aggregate_estimator_results(gt_second_results[:count // samples_per_gt])
...
for count in sample_counts:
    # Aggregate corresponding results.
    aggregated_results = aggregate_estimator_results(train_results[:count // samples_per_train])
    estimates = torch.tensor(aggregated_results['estimates']).float()
