...
train_loader = DataLoader(classifier_dataset, batch_size=mbsize, shuffle=True, drop_last=True, num_workers=0)
val_loader = DataLoader(classifier_dataset, batch_size=mbsize, shuffle=False, drop_last=False, num_workers=0)
...
filenames = np.sort(glob(os.path.join('data_files', dataset, f'mc-{num_points}-{num_points}-*.npy')))
