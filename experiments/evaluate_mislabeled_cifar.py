...
parser.add_argument('--mc_contributions', type=int, nargs='+', default=[5, 10, 25, 50, 100])
...
inference_dataloader = DataLoader(inference_dataset, batch_size=256, shuffle=False, drop_last=False, num_workers=0)
inference_dset = TensorDataset(torch.stack([inference_dataset[i][0] for i in range(len(inference_dataset))]))
inference_dloader = DataLoader(inference_dset, batch_size=256, shuffle=False, drop_last=False, num_workers=0)
...
for count in amortized_contributions:
    ...
    results = torch.load(os.path.join('model_results', dataset, f'amortized-50000-{count}.pt'))
    ...
    estimates = torch.cat(trainer.predict(checkpoint, inference_dloader)).squeeze().cpu().float()
...
plt.plot(range(1, x_max), torch.cumsum(positions.float(), 0)[:x_max-1],
...
for count, positions in mc_mislabeled_position_dict.items():
    plt.plot(range(1, x_max), torch.cumsum(positions.float(), 0)[:x_max-1],
             label=f'Monte Carlo ({count})', linestyle='--')
...
plt.figure()
mislabeled_scores = ground_truth_estimates[ground_truth_inds][mislabeled[ground_truth_inds]]
correct_scores = ground_truth_estimates[ground_truth_inds][~mislabeled[ground_truth_inds]]
...
data = [{
    'Overall < 0': (ground_truth_estimates[ground_truth_inds] < 0).float().mean().item(),
    'Mislabeled < 0': (ground_truth_estimates[ground_truth_inds][mislabeled[ground_truth_inds]] < 0).float().mean().item(),
    'Correct < 0': (ground_truth_estimates[ground_truth_inds][~mislabeled[ground_truth_inds]] < 0).float().mean().item(),
    'Mislabeled AUROC': binary_auroc(-ground_truth_estimates[ground_truth_inds], mislabeled[ground_truth_inds].int()).item(),
    'Mislabeled AUPRC': binary_auprc(-ground_truth_estimates[ground_truth_inds], mislabeled[ground_truth_inds].int()).item(),
    'Overall max true': 0,
...
for estimates, estimates_all in zip(ao_estimates, ao_estimates_all):
    data.append({
        ...
        'Overall max true': (estimates_all.argmax(dim=1) == y_train_true).float().mean().item(),
        'Mislabeled max true': (estimates_all.argmax(dim=1) == y_train_true)[mislabeled].float().mean().item(),
        'Correct max true': (estimates_all.argmax(dim=1) == y_train_true)[~mislabeled].float().mean().item(),
    })
