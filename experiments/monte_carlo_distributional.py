...
parser.add_argument(
    "-dataset", type=str, choices=["cifar10-10", "cifar10-25", "cifar10-50"]
)
...
if num_inds == num_points:
    ...
    relevant_inds = np.concatenate(
        [np.arange(num_inds // 2), np.arange(num_points - num_inds // 2, num_points)]
    )
