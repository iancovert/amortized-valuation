...
parser.add_argument(
    ...
    type=str,
    default="cifar10-10",
    choices=["cifar10-10", "cifar10-25", "cifar10-50"],
)
...
train_dataset = torch.load(os.path.join("data_files", dataset, "processed.pt"))[
    "train_dataset"
]
...
for max_epochs in (10, 20, 30):
    for lr, min_lr in [(1e-3, 1e-5), (2e-4, 1e-5)]:
        ...
        classifier.load_state_dict(
            torch.load(os.path.join("model_results", dataset, "resnet18_pretrained.pt"))
        )
        ...
        if loss < best_loss:
            print(
                f"New best model: max_epochs = {max_epochs}, lr = {lr}, min_lr = {min_lr}"
            )
            best_loss = loss
